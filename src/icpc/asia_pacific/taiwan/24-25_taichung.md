...
首先可以把 $\Arr{s}$ 離散化到 $[1, n]$，相同數字就以位置來排序。

先做前綴跟先做後綴的兩種狀況需要分開討論，不過先做後綴再做前綴剛好相當於對 $t_i = n - s_{n-i}$ 先做前綴再做後綴。以下只考慮先做前綴的情況。

假設想要一次排好 $[1, k]$ 的 **數字**，那麼前綴操作就要做到 $\max_{1 \le i \le k} \text{pos}_i$ 的位置，而因為 $k+1$ 並沒有保證排好，後綴就要從 **位置** $k+1$ 開始排序。
